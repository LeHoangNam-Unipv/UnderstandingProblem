...
def split_balance_class_data(valid_data, test_size=0.2):
    ...
    data_q1 = pd.DataFrame(columns=['User_ID', 'Label', 'Source_Question'])
    data_q2 = pd.DataFrame(columns=['User_ID', 'Label', 'Source_Question'])
    data_q3 = pd.DataFrame(columns=['User_ID', 'Label', 'Source_Question'])
    ...
    for q in QUESTION:
        ...
        if (q == 1):
            data_q1 = pd.concat([data_q1, question])
        elif(q == 2):
            data_q2 = pd.concat([data_q2, question])
        else:
            data_q3 = pd.concat([data_q3, question])


    # Split users into training and testing sets
    # Question 1 has too few 0 label so it cannot use stratify
    train_data_q1, test_data_q1 = train_test_split(data_q1, test_size=test_size)
    train_data_q2, test_data_q2 = train_test_split(data_q2, stratify=data_q2['Label'], test_size=test_size)
    train_data_q3, test_data_q3 = train_test_split(data_q3, stratify=data_q3['Label'], test_size=test_size)

    train_data = pd.concat([train_data_q1, train_data_q2, train_data_q3], ignore_index=True)
    # After split training and testing set, the class is not balanced yet, so we need to do that

    # Balance the test set
    if (TEST_QUESTION == 3):
        #balanced_test_data = balance_classes(test_data_q3, 'Label')
        balanced_test_data = test_data_q3
    elif(TEST_QUESTION == 2):
        #balanced_test_data = balance_classes(test_data_q2, 'Label')
        balanced_test_data = test_data_q2
...
#def main():
    #X_train, y_train, X_test, y_test = data_generation(750)
